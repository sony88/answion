...
class topic_class extends AWS_MODEL
{
	...
	public function get_topic_search_list($count = false, $query_data = null)
	{
		...
		if ($keyword)
		{			
			$where[] = "(MATCH(topic_title_fulltext) AGAINST('" . $this->model('search_index')->encode_search_code($this->model('system')->analysis_keyword($keyword)) . "' IN BOOLEAN MODE))";
		}
	...
	public function save_topic($question_id, $topic_title, $uid, $topic_lock = 0, $topic_type = 1)
	{
		if (! $uid)
		{
			return false;
		...
		if (!$topic_id = $this->get_topic_id_by_title($topic_title))
		{
	...
	public function remove_topic_by_ids($topic_id)
	{
		...
		foreach($topic_ids as $topic_id)
		{
			...
			//删除动作
			ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_TOPIC . ' AND associate_id = ' . $topic_id);
			ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION . ' AND associate_action = ' . ACTION_LOG::ADD_TOPIC . ' AND associate_attached = ' . $topic_id);
			
			$this->delete('topic_focus', 'topic_id = ' . $topic_id);
			$this->delete('topic_question', 'topic_id = ' . $topic_id);
			$this->delete('feature_topic', 'topic_id = ' . $topic_id);
			$this->delete('related_topic', 'topic_id = ' . $topic_id . ' OR related_id = ' . $topic_id);
			$this->delete('reputation_topic', ' topic_id = ' . $topic_id);
		...
		foreach ($log_list as $key => $log)
		{
			...
			switch ($log['associate_action'])
			{
				...
				case ACTION_LOG::ADD_RELATED_TOPIC : //添加相关话题
					...
					$title_list = '<a href="' . $user_url . '">' . $user_name . '</a> 添加了相关话题<p><a href="topic/' . rawurlencode($topic_info['topic_title']) . '">' . $topic_info['topic_title'] . '</a></p>';
					break;
				
				case ACTION_LOG::DELETE_RELATED_TOPIC : //删除相关话题
					$topic_info = $this->model('topic')->get_topic_by_id($log['associate_attached']);
					
					$title_list = '<a href="' . $user_url . '">' . $user_name . '</a> 删除了相关话题<p><a href="topic/' . rawurlencode($topic_info['topic_title']) . '">' . $topic_info['topic_title'] . '</a></p>';
					break;
			}
	...
	public function get_topic_action_list($topic_ids, $limit, $best_answer = false)
	{
		if ($best_answer)
		{
			...
			if (!$best_answers = $this->query_all("SELECT best_answer FROM " . $this->get_table('question') . " WHERE question_id IN (" . implode(',', $question_ids) . ")"))
			{
				return false;
			}
