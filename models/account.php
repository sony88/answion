...
{	
	 ...
	 * @return bool
	 */
	function update_notification_setting_fields($update_data, $userid)
	{
		$userid = intval($userid);
		...
		{
			$update_data['uid'] = $userid;
			
			$this->insert('users_notification_setting', $update_data);
		}
		else
		{
			$this->update('users_notification_setting', $update_data, 'uid = ' . $userid);
	...
	*/
	...
	function get_users_search_by_name($name)
	{
		return $this->fetch_row('users_search', "name = '" . $this->quote($name) . "'");
	}
	
	function delete_users_search_by_id($id)
	{
		return $this->delete('users_search', 'id = ' . intval($id));
	}
	
	function save_users_search($name, $search_data)
	{
		return $this->insert('users_search', array(
			'name' => $name,
			'search_data' => serialize($search_data),
		));
	}
	
	function update_users_search($id, $search_data)
	{
		return $this->update('users_search', array('search_data' => serialize($search_data)), 'id = ' . intval($id));
	}
	
	function get_users_search_list()
	{
		if ($rs = $this->fetch_all('users_search', null, 'id DESC'))
		{
			foreach($rs as $key => $val)
			{
				if ($rs[$key]['search_data'] = unserialize($val['search_data']))
				{
					$url_param = array();
					
					foreach($rs[$key]['search_data'] as $rkey => $rval)
					{
						$url_param[] = $rkey . '-' . $rval;
					}
					
					$rs[$key]['search_param'] = implode('__', $url_param);
				}
			}
