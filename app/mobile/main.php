...
class main extends AWS_CONTROLLER
{
	...
	public function question_action()
	{
		...
		{
			H::redirect_msg('问题不存在或已被删除', '/mobile/explore/');
		}
		...
		if (is_numeric($_GET['rf']) and $_GET['rf'])
		{
			...
			{
				$redirect_message[] = '从问题 <a href="' . get_js_url('/mobile/question/' . $_GET['rf'] . '?rf=false') . '">' . $from_question['question_content'] . '</a> 跳转而来';
			}
		}
		...
		else if ($question_info['redirect'])
		{
			if ($target_question)
			{
				$message = '此问题将跳转至 <a href="' . get_js_url('mobile/question/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']) . '">' . $target_question['question_content'] . '</a>';
				...
				{
					$message .= '&nbsp; (<a href="javascript:;" onclick="' . addslashes('ajax_request(G_BASE_URL + \'/question/ajax/redirect/\', \'item_id=' . $question_id . '\');') . '">撤消重定向</a>)';
				}
	...
	public function people_action()
	{
		...
		$this->crumb($user['user_name'] . ' 的个人主页', '/mobile/people/' . $user['url_token']);
	...
	public function inbox_action()
	{
		if ($_GET['dialog_id'])
		{
			...
			if ($dialog_id == 0)
			{
				H::redirect_msg('指定的站内信不存在', '/mobile/inbox/');
			}
			...
			$this->crumb('私信对话: ' . $recipient_user['user_name'], '/mobile/inbox/dialog_id-' . $dialog_id);
